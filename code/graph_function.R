graph.fun = function(mod, file_list){
 for (fl in file_list){
   if (mod=='RF'){
     tr = paste('./result/tuning/rf_train_result_',fl, sep='')
     vl = paste('./result/tuning/rf_val_result_', fl, sep='')
     ...
     fn = paste('./plots/rf_ntree_', strsplit(fl, split= ".csv"),'.png',sep='')
     ggsave(fn, plot = gg)
     ...
     fn = paste('./plots/rf_mtry_',strsplit(fl, split= ".csv"),'.png',sep='')
     ggsave(fn, plot = gg)
   }
    
   if (mod=='XG'){
     #### nrounds ####
     tr = paste('./result/tuning/xg_train_result_',fl,sep='')
     vl = paste('./result/tuning/xg_val_result_',fl,sep='')
     ...
     fn = paste('./plots/xg_nrounds_',strsplit(fl, split= ".csv"),'.png',sep='')
     ggsave(fn, plot = gg)
     ...
     gg<-gg+geom_vline(xintercept=ab.lst$ab,linetype='dashed',size=0.2)
     gg<-gg+theme(axis.text.x = element_text(size=6))
     fn = paste('./plots/xg_max_depth_',strsplit(fl, split= ".csv"),'.png',sep='')
     ggsave(fn, plot = gg)
     ...
     gg<-gg+geom_vline(xintercept=ab.lst$ab,linetype='dashed',size=0.2)
     gg<-gg+theme(axis.text.x = element_text(size=6))
     fn = paste('./plots/xg_eta_',strsplit(fl, split= ".csv"),'.png',sep='')
     ggsave(fn, plot = gg)
